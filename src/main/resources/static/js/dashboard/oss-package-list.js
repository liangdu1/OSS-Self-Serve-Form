...
function GenerateTransactionListUI_OSS(tabName) {
    ...
    for (i = 0; i < transactionList_OSS.length; i++) {
        ...
        var id = transactionList_OSS[i].id;
        htmlTransactionList += '<div > ';
        htmlTransactionList += '<a href="/a/packages/' + id + '"  style="width: 100%;"> ';
        htmlTransactionList += '<div class="row list-table-row-oss waves-effect" id="' + id + '" style="padding-left:15px;padding-right:15px;padding-top:5px;padding-bottom:5px;margin: 0px;">';
        ...
        htmlTransactionList += '    </div>';
        htmlTransactionList += '    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 list-table-cell-oss">';
    ...
    htmlTransactionList += '<ul class="pagination" style="padding-left:20px;">';

    var startNum = transactionList_CurrentPage_OSS > 3 ? transactionList_CurrentPage_OSS - 3 : 1;
    var endNum = transactionList_CurrentPage_OSS > 3 ? transactionList_CurrentPage_OSS + 3 : 7;


    for (var ic = startNum - 1; ic < endNum ; ic++) {

        if ((ic + 1) == transactionList_CurrentPage_OSS) {
            htmlTransactionList += '    <li class="active " >';
        } else {
            htmlTransactionList += '    <li >';
        }
        htmlTransactionList += '    <a class="waves-effect" href="#" onclick="TransactinoListPageTo_OSS(' + (ic + 1) + ');return false;">' + (ic + 1) + '</a>';
        htmlTransactionList += '    </li>';
    }
    ...
    $("#div_" + tabName + "_TransactionList_content_OSS").html(htmlTransactionList);
...
//need to change to an ajax request
function TransactinoListPageTo_OSS(pageTo) {
    ...
    transactionList_CurrentPage_OSS = pageTo;
    bulkRequestPackages(transactionList_CurrentPage_OSS, transactionList_PageSize_OSS, transactionList_Status_OSS);
}
