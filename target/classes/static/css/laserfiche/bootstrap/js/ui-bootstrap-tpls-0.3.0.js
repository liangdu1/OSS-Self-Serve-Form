...
angular.module('ui.bootstrap.dropdownToggle', []).directive('dropdownItem', [function () {
    // we need to signal that an item was clicked so the menu can know to close
    // jQuery will just remove the class, but we need to update the model
    return {
        link: function (scope, element, attrs) {
            element.click(function () {
                scope.$emit('itemClicked');
            });
        }
    };
}]).directive('dropdownToggle',
  ['$document', '$location', '$window', '$timeout', function ($document, $location, $window, $timeout) {
      var openElement = null,
          closeMenu = angular.noop,
          delay = null; // result from last $timeout()
      return {
          restrict: 'CA',
          link: function (scope, element, attrs) {
              scope.$watch('$location.path', function () { closeMenu(); });
              scope.$on('itemClicked', function () { openElement = null; });
              element.parent().bind('click', function () { closeMenu(); });
              element.hover(function () {
                  if (openElement != element && openElement != null) {
                      // if we're hovering and another one is open, close the open one and open this one
                      closeMenu();
