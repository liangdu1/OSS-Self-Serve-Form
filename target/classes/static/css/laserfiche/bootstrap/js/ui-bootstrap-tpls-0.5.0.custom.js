  ...
  return {
    ...
    link: function(originalScope, element, attrs, ngModel) {
     ...
     // create a child scope for the datepicker directive so we are not polluting original scope
      ...
      // Outter change
      scope.$watch(function bootstrapModelValueWatch() {
...
angular.module('ui.bootstrap.dropdownToggle', []).directive('dropdownToggle', ['$document', '$location', function ($document, $location) {
  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      $document.bind('contextmenu', function () { closeMenu(); });
      element.bind('click', function (event) {
        ...
        if (!elementWasOpen) {
          ...
          openElement = element;
          openElement.attr('aria-expanded', 'true'); // WCAG
          ...
          closeMenu = function (event) {
            ...
            closeMenu = angular.noop;
            openElement.attr('aria-expanded', 'false'); // WCAG
            openElement = null;
          };
          $document.bind('click', closeMenu);
        }
      });
      element.attr('aria-expanded', 'false'); // WCAG
    }
  };
}]);
      ...
      return function $tooltip(type, prefix, defaultTriggerShow) {
          ...
          var template =
            '<div ' + directiveName + '-popup ' +
              ...
              'custom-class="' + startSym + 'tt_class' + endSym + '" ' +
              'is-open="tt_isOpen"' +
              '>' +
            '</div>';
              ...
              compile: function (tElem, tAttrs) {
                  ...
                  return function link(scope, element, attrs) {
                          ...
                          switch (scope.tt_placement) {
                              case 'right':
                                  ttPosition = {
                                      ...
                                      left: Math.max(position.left + position.width, 10)
                                  };
                                  break;
                              case 'bottom':
                                  ttPosition = {
                                      top: position.top + position.height,
                                      left: Math.max(position.left + position.width / 2 - ttWidth / 2, 10)
                                  };
                                  if (ttPosition.left == 10)
                                      $(tooltip[0].children[0]).css({ left: (position.left + position.width / 2 - 10) + 'px' });
                                  break;
                              case 'left':
                                  ttPosition = {
                                      top: position.top + position.height / 2 - ttHeight / 2,
                                      left: Math.max(position.left - ttWidth, 10)
                                  };
                                  break;
                              default:
                                  ttPosition = {
                      ...
                      function show() {
                          ...
                          // visible yet anyway.
                          if (appendToBody) {
                              // Force the z-index to be higher than dialog modal body
                      ...
                      attrs.$observe(prefix + 'Image', function (val) {
                          scope.tt_image = angular.isDefined(val) ? val : "";
                      });
                      ...
                      // SCR 138567, due to new angularJS observe will not fire if undefined
                      // https://github.com/angular/angular.js/commit/6f3b8622adce2006df5cf7eed4bf9262539004bd
                      // do the fix here since the newer bootstrap should take care of it already
                      ...
                      scope.$on('TooltipImageLoaded', function () {
                          positionTooltip();
                      });
                  };
              }
...
.directive('tooltipPopup', function () {
    return {
        restrict: 'EA',
        replace: true,
        scope: { content: '@', image: '@', placement: '@', image: '@', animation: '&', isOpen: '&', customClass: '@' },
...
.directive('tooltipHtmlUnsafePopup', function () {
    return {
        restrict: 'EA',
        replace: true,
        scope: { content: '@', placement: '@', animation: '&', isOpen: '&', customClass: '@' },
...
angular.module("template/tooltip/tooltip-html-unsafe-popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tooltip/tooltip-html-unsafe-popup.html",
    ...
    "  <div class=\"tooltip-inner\" compile=\"content\"></div>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("template/tooltip/tooltip-popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/tooltip/tooltip-popup.html",
    "<div class=\"tooltip {{placement}} {{customClass}}\" ng-class=\"{ in: isOpen(), fade: animation() }\">\n" +
    "  <div class=\"tooltip-arrow\"></div>\n" +
    "  <div class=\"tooltip-inner\" ng-if=\"image == ''\" ng-bind=\"content\"></div>\n" +
    "  <div class=\"tooltip-inner-image\" ng-if=\"image != ''\">\n" +
    "     <img ng-src=\"{{image}}\" style=\"width: auto; height: auto\" spinner-on-load />\n" +
    "  </div>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("template/timepicker/timepicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/timepicker/timepicker.html",
    ...
    "	</tr>\n" +
    "	<tr>\n" +
    "		<td class=\"control-group\" ng-class=\"{'error': !validHours}\"><input type=\"text\" ng-model=\"hours\" ng-change=\"updateHours()\" class=\"span1 timeValue text-center\" ng-mousewheel=\"incrementHours()\" ng-readonly=\"readonlyInput\" maxlength=\"2\" /></td>\n" +
    "		<td>:</td>\n" +
    "		<td class=\"control-group\" ng-class=\"{'error': !validMinutes}\"><input type=\"text\" ng-model=\"minutes\" ng-change=\"updateMinutes()\" class=\"span1 timeValue text-center\" ng-readonly=\"readonlyInput\" maxlength=\"2\"></td>\n" +
    "		<td ng-show=\"showMeridian\"><button ng-click=\"toggleMeridian()\" class=\"btn changeMeridian text-center\">{{meridian}}</button></td>\n" +
    "	</tr>\n" +
    "	<tr class=\"text-center\">\n" +
